...
static bool smcdsd_lcm_path_lock(bool locking)
{
	...
	call_drv_ops(plcd, lock, locking);
	...
	case MIPI_DSI_DCS_SHORT_WRITE:
	{
		...
		if (plcd->cmd_q) {
			...
			if (lcm_util.dsi_set_cmdq_V22) {
				...
				dbg_info("%s : lcm_util.dsi_set_cmdq_V22 is not exist\n", __func__);
				ret = -EINVAL;
			}
		}
		break;
	...
	bool need_lock)
{
	struct mipi_dsi_lcd_common *plcd = drvdata ? drvdata : get_lcd_common(0);
	int ret = 0;
#ifdef CONFIG_SMCDSD_USE_POINT_GPARA
	u8 gpara_buf[4] = {0xB0, 0x00, 0x00, 0x00};
#endif
	if (!plcd->lcdconnected)
		return ret;
	...
	if (offset) {
		...
		gpara_buf[3] = cmd;
		smcdsd_panel_dsi_command_tx(drvdata, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)gpara_buf, 4, need_lock);
		dbg_info("%s: %*ph\n", __func__, ARRAY_SIZE(gpara_buf), gpara_buf);
	}
#endif
	if (plcd->cmd_q)
		...
		if (lcm_util.dsi_dcs_read_lcm_reg_v2) {
			...
			dbg_info("%s : lcm_util.dsi_dcs_read_lcm_reg_v2 is not exist\n", __func__);
			ret = -EINVAL;
		}
	}
