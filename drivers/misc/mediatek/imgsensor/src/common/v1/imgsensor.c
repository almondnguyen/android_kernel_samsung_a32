...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	static bool get_search_list;
	static bool init_flag_search_list;

	unsigned int i = 0;
	unsigned int j = 0;
	char *driver_name = NULL;

	static const char * const config_sensor_name[] = {
	...
	static const char * const config_2nd_sensor_name[] = {
...
#if defined(CONFIG_CAMERA_AAU_V32)
		const int HEADER_SIZE = sizeof(unsigned short);
		int cal_read_size	= buf_size - HEADER_SIZE;

		// EEPROM_crosstalk_offset for A32 LTE: GW3, HI2021Q
		const int EEPROM_OFFSET_ARR[][2] = {
			{0x0170, 0x4DF}, // EEPROM_crosstalk_offset for A32-LTE rear sensor gw3 - IMGSENSOR_SENSOR_IDX_MAIN
			{0x09D0, 0x0BFF}  // EEPROM_crosstalk_offset for A31, A32-LTE front sensor hi2021q - IMGSENSOR_SENSOR_IDX_SUB
		};
...
static int imgsensor_probe(struct platform_device *pdev)
{
	...
	cam_info_probe(pdev->dev.of_node);
