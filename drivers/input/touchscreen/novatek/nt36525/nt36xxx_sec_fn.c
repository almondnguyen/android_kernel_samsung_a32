...
static void clear_cover_mode(void *device_data)
{
	...
	if (ts->power_status == POWER_OFF_STATUS) {
		if (cmd == HOLSTER_LEAVE) {
			input_info(true, &ts->client->dev, "%s: before ts->sec_function : 0x%02x\n", __func__, ts->sec_function);
			ts->sec_function &= ~HOLSTER_MASK;
			input_info(true, &ts->client->dev, "%s: after ts->sec_function : 0x%02x\n", __func__, ts->sec_function);
		} else {
			input_info(true, &ts->client->dev, "%s: before ts->sec_function : 0x%02x\n", __func__, ts->sec_function);
			ts->sec_function |= HOLSTER_MASK;
			input_info(true, &ts->client->dev, "%s: after ts->sec_function : 0x%02x\n", __func__, ts->sec_function);
		}
		input_err(true, &ts->client->dev, "%s: POWER_STATUS : OFF It'll be set after screen on.!\n", __func__);
		goto out_ok;
	}
