...
int check_yas539_adc_data_spec(int type)
{
	...
	if ((event->x == 0) && (event->y == 0) && (event->z == 0)) {
		return -1;
	} else if ((event->x > GM_DATA_SPEC_MAX) || (event->x < GM_DATA_SPEC_MIN) || (event->y > GM_DATA_SPEC_MAX) ||
		   (event->y < GM_DATA_SPEC_MIN) || (event->z > GM_DATA_SPEC_MAX) || (event->z < GM_DATA_SPEC_MIN)) {
		return -1;
	} else {
		return 0;
	}
