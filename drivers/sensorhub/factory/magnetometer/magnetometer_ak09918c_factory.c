...
int check_ak09918c_adc_data_spec(int type)
{
	...
	if ((event->x == 0) && (event->y == 0) && (event->z == 0)) {
		return -1;
	} else if ((event->x >= GM_AKM_DATA_SPEC_MAX) || (event->x <= GM_AKM_DATA_SPEC_MIN) ||
		   (event->y >= GM_AKM_DATA_SPEC_MAX) || (event->y <= GM_AKM_DATA_SPEC_MIN) ||
		   (event->z >= GM_AKM_DATA_SPEC_MAX) || (event->z <= GM_AKM_DATA_SPEC_MIN)) {
		return -1;
	} else if ((int)abs(event->x) + (int)abs(event->y) + (int)abs(event->z) >= GM_DATA_SUM_SPEC) {
		return -1;
	} else {
		return 0;
	}
...
Retry_selftest:
	...
	do {
		msleep(60);
		if (check_ak09918c_adc_data_spec(SENSOR_TYPE_GEOMAGNETIC_POWER) == 0) { // success
			break;
		}
