...
int sensorhub_shutdown()
{
	scp_A_unregister_notify(&scp_state_notifier);
	return 0;
}

int sensorhub_refresh_func()
{
	int ret;
...
void check_refresh_task()
{
	struct shub_data_t *data = get_shub_data();

	if (data->complete_refresh_task)
		return;
	shub_infof("refresh task");
	queue_refresh_task();
}

int sensorhub_comms_write(u8 *buf, int length, int timeout)
{
