...
try_write:
	if (len == 1)
		ret = lcd->pdata->tx(lcd->pdata, MIPI_DSI_DCS_SHORT_WRITE, (unsigned long)cmd, len, need_lock);
	else if (len == 2)
		ret = lcd->pdata->tx(lcd->pdata, MIPI_DSI_DCS_SHORT_WRITE_PARAM, (unsigned long)cmd, len, need_lock);
	else
...
static int td4150_read_id(struct lcd_info *lcd, bool need_lock)
{
	int i, ret = 0;
...
static int td4150_read_init_info(struct lcd_info *lcd, bool need_lock)
{
	int ret = 0;
...
static int td4150_exit(struct lcd_info *lcd)
{
	...
	DSI_WRITE(SEQ_TD4150_DISPLAY_OFF, ARRAY_SIZE(SEQ_TD4150_DISPLAY_OFF));

	msleep(20);

	DSI_WRITE(SEQ_TD4150_SLEEP_IN, ARRAY_SIZE(SEQ_TD4150_SLEEP_IN));
...
static int td4150_init(struct lcd_info *lcd)
{
	...
	td4150_read_init_info(lcd, false);
	...
	DSI_WRITE(SEQ_TD4150_SLEEP_OUT, ARRAY_SIZE(SEQ_TD4150_SLEEP_OUT));
	msleep(80);
	DSI_WRITE(SEQ_TD4150_DISPLAY_ON, ARRAY_SIZE(SEQ_TD4150_DISPLAY_ON));
	DSI_WRITE(SEQ_TD4150_BRIGHTNESS_MODE, ARRAY_SIZE(SEQ_TD4150_BRIGHTNESS_MODE));
	DSI_WRITE(SEQ_TD4150_BRIGHTNESS, ARRAY_SIZE(SEQ_TD4150_BRIGHTNESS));
...
static int td4150_probe(struct lcd_info *lcd)
{
	...
	td4150_read_init_info(lcd, true);
	...
	/* smcdsd_panel_set_brightness(lcd, 1); */	/* todo */
...
__XX_ADD_LCD_DRIVER(td4150_mipi_lcd_driver);
