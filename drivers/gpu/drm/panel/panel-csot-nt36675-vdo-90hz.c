...
#define ENABLE_MTK_LCD_HWID_CHECK
/*******************************************************/
/* Backlight Function                                  */
/* Device Name : LM36274                               */
/* Interface   : I2C                                   */
...
static int _lcm_lcd_id_dt(struct device *dev, int *lcd_id)
{
	struct device_node *np_lcm_id = dev->of_node;
	...
	if (np_lcm_id == NULL) {
		pr_info("np_lcm_id dev node is NULL\n");
		return -1;
	}

	_lcd_id_gpio1 = of_get_named_gpio(np_lcm_id, "i2c_lcd_bias,lcd_id_1", 0);
	_lcd_id_gpio2 = of_get_named_gpio(np_lcm_id, "i2c_lcd_bias,lcd_id_2", 0);
	_lcd_id_gpio3 = of_get_named_gpio(np_lcm_id, "i2c_lcd_bias,lcd_id_3", 0);

	/* TO DO : read gpio value */
	lcd_id = (gpio_get_value(_lcd_id_gpio3) << 2) | (gpio_get_value(_lcd_id_gpio2) << 1) | gpio_get_value(_lcd_id_gpio1);
			  ...
			  const struct i2c_device_id *id)
{
	...
	_lcm_lcd_id_dt(&client->dev, &lcd_id);
	...
	if ((hw_version >= BOARD_HW_REV_02 ) && (lcd_id == LCD_CSOT_NTW36675_ID)) {
...
#ifdef ENABLE_MTK_LCD_HWID_CHECK
	struct gpio_desc *lcd_id1, *lcd_id2, *lcd_id3;
#endif
	bool prepared;
	bool enabled;
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	ret = of_property_read_u32(dev->of_node, "lcdid_num", &lcdid_num);
	if (ret < 0) {
		pr_info("get lcd hw id num fail:%d\n", ret);
		lcdid_num = 0;
	} else {
		pr_info("lcd hw id num = %d\n", lcdid_num);
	}

#ifdef ENABLE_MTK_LCD_HWID_CHECK
	ctx->lcd_id1 = devm_gpiod_get_index(ctx->dev,"lcdid", 0, GPIOD_IN);
	if (IS_ERR(ctx->lcd_id1)) {
		dev_err(ctx->dev, "%s: cannot get lcd_id1 %ld\n",
			__func__, PTR_ERR(ctx->lcd_id1));
		return PTR_ERR(ctx->lcd_id1);
	}
	lcd_id_pins |= (gpiod_get_value(ctx->lcd_id1));
	devm_gpiod_put(ctx->dev, ctx->lcd_id1);

	ctx->lcd_id2 = devm_gpiod_get_index(ctx->dev,"lcdid", 1, GPIOD_IN);
	if (IS_ERR(ctx->lcd_id2)) {
		dev_err(ctx->dev, "%s: cannot get lcd_id2 %ld\n",
			__func__, PTR_ERR(ctx->lcd_id2));
		return PTR_ERR(ctx->lcd_id2);
	}
	lcd_id_pins |= (gpiod_get_value(ctx->lcd_id2) << 1);
	devm_gpiod_put(ctx->dev, ctx->lcd_id2);

	ctx->lcd_id3 = devm_gpiod_get_index(ctx->dev,"lcdid", 2, GPIOD_IN);
	if (IS_ERR(ctx->lcd_id3)) {
		dev_err(ctx->dev, "%s: cannot get lcd_id3 %ld\n",
			__func__, PTR_ERR(ctx->lcd_id3));
		return PTR_ERR(ctx->lcd_id3);
	}
	lcd_id_pins |= (gpiod_get_value(ctx->lcd_id3) << 2);
	devm_gpiod_put(ctx->dev, ctx->lcd_id3);

	if (lcd_id_pins != lcdid_num)
		return -1;
#endif
